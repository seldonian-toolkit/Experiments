...
def generate_behavior_policy_episodes(hyperparameter_and_setting_dict,n_trials,save_dir,verbose=False):
    ...
    for trial_i in range(n_trials):
        if verbose: print(f"Trial: {trial_i+1}/{n_trials}")
...
def update_spec_with_best_hyperparams(trial_i, data_frac, spec_for_exp, hyperparam_spec, 
    ...
    # TODO: Update way to check if hyperparameters changed, bc ran run new trial but sitll same.
    updated_hyperparams = ran_new_bs_trials

    return spec_for_exp, updated_hyperparams
