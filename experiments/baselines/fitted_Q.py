...
class TabularFittedQBaseline(BaseFittedQBaseline):
    def __init__(self,model_name,regressor_class,policy,num_iters=100,env_kwargs={}):
...
# class FunctionalFittedQBaseline(TabularFittedQBaseline):
#     def __init__(self,regressor_class,policy,num_iters=100,env_kwargs={}):
#         """Implements fitted-Q RL baseline for a Q table but with 
#         a functional regressor. In each iteration, uses the fitted regressor
#         to approximate the Q values and fills those in the Q table. 
#         """
#         super().__init__(regressor_class,policy,num_iters,env_kwargs)
       
#     def get_regressor_weights(self):
#         """ Approximates Q table by passing each possible input
